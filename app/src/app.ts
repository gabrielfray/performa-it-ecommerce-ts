...
const sizeInputs = document.querySelectorAll<HTMLInputElement>(".size-input");
...
class Product {
    private product: ItemCarrinho;

    constructor(product: ItemCarrinho) {
    ...
    private createParagraph(price: number, promotionPrice: number): HTMLParagraphElement {
        const paragraph: HTMLParagraphElement = document.createElement("p");
        paragraph.innerHTML = `${this.getFormattedPrice(price)} <strong>${this.getFormattedPrice(promotionPrice)}</strong>`;
        return paragraph;
    }

    private createInstallmentSpan(price: number): HTMLSpanElement {
        const installmentValue: string = (price / 3).toFixed(2);
        const installmentSpan: HTMLSpanElement = document.createElement("span");
        installmentSpan.innerHTML = `ou 3x de R$ ${installmentValue}`;
        return installmentSpan;
    }

    createContentClothes(): HTMLDivElement {
        const contentClothes: HTMLDivElement = document.createElement("div");
        contentClothes.classList.add("content-clothes")

        const clotheImage: HTMLImageElement = document.createElement("img");
        ...
        const contentPrice: HTMLDivElement = document.createElement("div");
        ...
        const clothesName: HTMLDivElement = document.createElement("div");
        clothesName.className = "name";

        clothesName.append(this.createParagraph(
            this.product.price, this.product.promotionPrice),
            this.createInstallmentSpan(this.product.price)
        );
...
function displayProducts(database: ItemCarrinho[]): void {
    const recommendedClothes = document.querySelector(".recommended-clothes");

    if (!recommendedClothes) return;
...
class ProductDisplay {
    private mainClothe: HTMLImageElement | null;
    private asideClothesList: NodeListOf<HTMLImageElement>;
    private clotheName: HTMLElement | null;
    private clotheId: HTMLElement | null;
    private clotheOriginalPrice: HTMLElement | null;
    private clothePromotionPrice: HTMLElement | null;
    private clotheDivisionPrice: HTMLElement | null;
    private buttonBuy: HTMLElement | null;
    ...
    private displayProductDetails(product: ItemCarrinho): void {
    ...
    private showRandomProduct(): void {
...
class ShoppingCart {
    ...
    private cartFooter: HTMLElement | null;
    private emptyCart: HTMLElement | null;
    ...
    public addItem(product: ItemCarrinho): void {
    ...
    private createClothesCard(item: any): void {
        const contentClothes = document.querySelector(".content__list-clothes");
    ...
    private handleBuyButton(event: Event): void {
        ...
        if (clickTarget.classList.contains("button-buy")) {
            ...
            const colorInputs = document.querySelectorAll(".color-input") as NodeListOf<HTMLInputElement>;
    ...
    private handleQuantityButton(contentQuantityClothes: HTMLElement, quantityCount: HTMLElement, item: ItemCarrinho): void {
        if (contentQuantityClothes) {
            contentQuantityClothes.addEventListener("click", (event) => {
                const subtotalPrice: HTMLElement | null = document.querySelector(".subtotal");
    ...
    private calculateSubtotal(subtotalPrice: HTMLElement): void {
...
document.querySelectorAll(".toggle-cart").forEach((elem) => elem.addEventListener
    ("click", () => {
